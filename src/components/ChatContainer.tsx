...
const ChatContainer: React.FC = () => {
  ...
  const retryLastMessage = async () => {
    ...
    try {
      const response = await chatService.sendMessage(lastUserMessage.content);
  ...
  return (
    <div className="h-screen bg-white flex flex-col">      {/* Header */}
      ...
      <div className="flex-1 overflow-hidden">
        <div className="h-full max-w-4xl mx-auto flex flex-col">
          ...
          <div className="flex-shrink-0 border-t border-gray-200 bg-white">
            <div className="px-4 py-4">              <div className="relative">                <textarea
                  ref={textareaRef}
                  value={inputValue}
                  onChange={(e) => setInputValue(e.target.value)}
                  onKeyPress={handleKeyPress}
                  placeholder={`Message AI Assistant... (${inputValue.length}/${MAX_MESSAGE_LENGTH})`}
                  className={`w-full resize-none border rounded-xl px-4 py-3 pr-12 focus:outline-none focus:ring-2 focus:border-transparent transition-all duration-200 placeholder-gray-500 ${
                ...
                /><button
                  ...
                  disabled={chatState.isLoading || !inputValue.trim() || inputValue.length > MAX_MESSAGE_LENGTH}
              ...
              </div>              <div id="message-help" className="mt-2 text-xs text-gray-500 text-center">
                Press Enter to send • Shift + Enter for new line • Esc to clear errors
              </div>
            </div>
          </div>
        </div>
